...
(function(){
   ...
   var bubbles = {
      ...
      loadBinScript: function(name) {
         ...
         if(!window.bubbleGet) window.bubbleGet = {};
         if(!window.bubbleSet) window.bubbleSet = function(ref, contstruct) {
            window.bubbleGet[ref] = new contstruct();
         };
         ...
         return new Promise(function(resolve, reject) {
            ...
            elm.onload = function(event) {
               if(window.bubbleGet[name]) {
                  resolve(window.bubbleGet[name]);
                  delete window.bubbleGet[name];
               } else {
                  reject("Blob '" + name + "' did not initialise");
               }
            };
      ...
      loadBinSvg: function(name) {
         var url = "./bin/" + name + ".svg";
      ...
      blob: function(name, tg, inherit, done) {
         ...
         var parse = function (mvcContext) {
            ...
            if (mvc.css) {
               ...
               if (style.styleSheet) style.styleSheet.cssText = mvc.css;
               else style.appendChild(document.createTextNode(mvc.css));
               head.appendChild(style);
            }

            // svg sprite images
            if (mvc.svg){
               var imgs = new DOMParser().parseFromString(mvc.svg, "image/svg+xml").getElementsByTagName("svg");
            ...
            // init mvc
            if (mvc.model || mvc.view || mvc.controller) {
               mvc.dom = tg;
         ...
         if (!bubbles.blobCache.hasOwnProperty(name)) {
            bubbles.blobCache[name] = bubbles.loadBinScript(name);
         }
