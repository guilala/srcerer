...
(function(){
   ...
   var bubbles = {
         ...
         // child blob
         {
            ...
            act: function () {
               bubbles.arrayify(this.elm.blob).forEach(function (blobSrc) {
                  if(blobSrc.name) {
                     // insert
                     bubbles.blob(blobSrc.name, this.domElm, blobSrc, function (ctx) {
      ...
      // mvc
      mvc: function(tg, mvc, inherit, mvcDone) {
         // inheritance
         var ctx = {
            bubbles: bubbles,
            mvc: mvc,
         ...
         if(mvc.view && mvc.view.call) {
            que.add(function(next){
               ...
               // render view elm
               var root = bubbles.elm(viewElm, tg, ctx);

               // create view destroyer
         ...
         que.then(function(){
             if(mvcDone && mvcDone.call) mvcDone.call(ctx);
         });
      },
      ...
      blob: function(name, tg, ctx, done) {
