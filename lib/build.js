...
const confNginx = function(){
   ...
   if(nginx && this.appConf.modified) {
      ...
      .add((done) => {
         fs.readFile(nginx.template, "utf8", (err, data) => {
      ...
      .add((done) => {
         fs.ensureFile(nginx.file, function (err) {
            if(err) console.error("confNginx", err);
            else done();
         });
      })
      ...
      .then(() => {
         tpl = tpl
            ...
            .replace("<<errors>>", nginx.errors)
            .replace("<<srcerer>>", nginx.srcerer)
            .replace("<<app>>", nginx.root)
            .replace("<<crt>>", nginx.crt)
            .replace("<<key>>", nginx.key);

         fs.writeFile(nginx.file, tpl, "utf8", (err) => {
            if(err) console.error("confNginx", err);
