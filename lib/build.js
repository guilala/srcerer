...
const getBuildName = function(next) {
   var basePath = path.normalize(this.urlBase).split(path.sep);
   if(basePath.length) {
...
const confLess = function(compress) {
   return {
      ...
      filename: "Cannot parse less", // error message
      compress: compress
   };
};
...
const serveRoot = function(next) {
   // exit build if not requesting app index
   if(this.urlPath.length > 1 && this.urlPath != "/index.html") {
      this.exit();
   }

   // else mount static routes and build index
   else {
      // remove previous static routes
      this.runtime.router.stack.filter(route => {
         return route.name === "serveStatic";
      }).forEach(staticRoute => {
         this.runtime.router.stack.splice(this.runtime.router.stack.indexOf(staticRoute), 1);
      });

      // mount static files from source in debug mode
      if(this.debug && this.staticSrc) {
         this.runtime.router.use(this.urlBase, this.runtime.static(this.staticSrc));
      }

      // mount static www root, exterior assets in debug mode
      if(this.staticRoot) {
         this.runtime.router.use(this.urlBase, this.runtime.static(this.staticRoot));
      }

      // mount custom source paths
      if(util.obj.isArray(this.appConf.staticPaths)) {
         this.appConf.staticPaths.forEach(staticPath => {
            this.runtime.router.use(staticPath.url, this.runtime.static(staticPath.path));
         });
      }
...
const addBlob = function(blobName, blobPath, done) {
   ...
   // is mvc modified
   .add((done) => {
      util.modified(blob.mvcFile, this.build + blobName + "Js").then((state) => {
   ...
   // is css modified
   .add((done) => {
      util.modified(blob.cssFile, this.build + blobName + "Css").then((state) => {
   ...
   // is svg modified
   .add((done) => {
      util.modified(blob.svgFile, this.build + blobName + "Svg").then((state) => {
...
const blobMvc = function(done) {
   ...
   var parseBlob = (data) => {
      ...
      var mvcParsed = ugly.minify(this.mvcSrc, {
         warnings: true,
         mangle: true
      });

      assert.equal(mvcParsed.error);
      this.mvc = mvcParsed.code;
...
const indexWrite = function (next) {
   if(this.appConf.modified) {
      ...
      if(this.appConf.index) {
         ...
         // or write to static source
         else if(this.staticSrc) {
...
const confNginx = function(){
   ...
   if(nginx && this.appConf.modified) {
      ...
      .then(() => {
         ...
         .replace("<<srcerer-app>>", nginx.srcererApp)
         .replace("<<srcerer-io>>", nginx.srcererIo)
         .replace("<<srcerer-socket>>", nginx.srcererSocket);

         // replace any <<key>> from nginx.paths in template
         for(var key in nginx.paths) {
             tpl = tpl.replace("<<" + key + ">>", nginx.paths[key]);
         }
