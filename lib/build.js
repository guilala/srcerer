...
const serveRoot = function(next) {
   // exit if request is not for index
   if(this.urlPath.length > 1 && this.urlPath != "/index.html") {
      this.exit();
   }

   // else enable static route and build index
   else {
      // remove previous static route
      this.runtime.router.stack.filter(s => {
         return s.path && s.name === "serveStatic" && s.path.indexOf("/app/" + this.build) === 0;
      }).forEach(staticRoute => {
         this.runtime.router.stack.splice(this.runtime.router.stack.indexOf(staticRoute), 1);
      });

      // serve static files from source in debug mode
      if(this.debug && this.staticSrc) {
         this.runtime.router.use(this.urlBase, this.runtime.static(this.staticSrc));
      }

      // serve static www root, exterior assets in debug mode
      if(this.staticRoot) {
