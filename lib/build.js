...
if (module.parent) {
   ...
   var createIndexFile = function (req, res, app, next) {
      ...
      mainQue.push(function (mainResolve) {
         ...
         app.style.forEach(function (s) {
            try {
               style += into("$0\n", [fs.readFileSync(server.root + server.appsRoot + app.build + "/" + s, "utf8")]);
            } catch(e) {
               console.error(e);
            }
         });
         ...
         less.render(style, lessConfig, function (lessParseError, cssResult) {
            ...
            // javascripts to add to index file
            app.lib = app.lib || [];

            // attach lib files to app root
            for(var t=0; t<app.lib.length; t++){
               app.lib[t] = server.root + server.appsRoot + build + "/" + app.lib[t];
            };

            // bubbles.js is mandatory first inline script
            app.lib.unshift(__dirname + "/bubbles.js");

            // individual inline scripts
            if (app.lib.length) app.lib.forEach(function (file) {
               try {
                  var libTxt = fs.readFileSync(file, "utf8");
         ...
         if (server && path.length && build) {
            ...
            util.localJson(appConfPath).then(function(appData){
               ...
               // compile srcerer app
                  ...
                  app.build = build;
                  createIndexFile(req, res, app, next);
               }
            }, console.error);
         } else next();
      };
