...
const enableMountPoints = function(done) {
   var loadConfigurations = util.all(this);

   const mount = (configurationFilePath, handler, mountType) => {
      ...
      // keep configuration file path
      this.mount[mountType][mountPoint] = {
         
      };

      // load configuration
      loadConfigurations.add(function(confDone){
         util.localJson(
            configurationFilePath
         ).then((confData) => {
            if(confData.server && confData.server.indexOf && confData.server.indexOf(this.conf.name) >= 0) {
               // store configuration
               this.mount[mountType][mountPoint] = {
   ...
   .on("data", (item) => {
      ...
      else if(fileName === "io.json"){
         mount(item.path, io, "io");
      }

      else if(fileName === "socket.json"){
         mount(item.path, socket, "socket");
      }
   })

   .on("end", function(){
      loadConfigurations.then(done);
   });
};
