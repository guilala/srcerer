...
const configureExpress = function(done) {
   runtime.server = require("http").createServer();
   ...
   // fav icon
   if(runtime.conf.favicon) {
      runtime.express.use(favicon(runtime.conf.root + runtime.conf.favicon));
   }
...
const enableMountPoints = function(done) {
   const mount = function(mountPath, handler, mountType) {
...
const routeSockets = function(done) {
   if(runtime.conf.mountSockets) {
      ...
      runtime.wss.on("connection", function connection(ws) {
         if(ws.protocol){
            socket(ws, runtime);
         }
      });
...
const enableReply = function(done) {
   ...
   runtime.router.use(function (req, res, next) {
      if(util.obj.keyCount(res.respond)){
         ...
         res.send(
            util.obj.merge({
               io: `${runtime.conf.name} ${runtime.conf.version}`,
...
module.exports = function(confIn) {
   ...
   .then(function(){
      ...
      // start express
      //runtime.express.listen(, runtime.conf.domain);
      runtime.server.on("request", runtime.express);
      runtime.server.listen(runtime.conf.port, runtime.conf.domain, function() {
         var serverInstance = runtime.server.address();
