...
(function(){
   ...
   var mount = function (req, res) {
      console.log("blah", req.path);
      res.send("Blah");
   }
   ...
   mainQue.push(function(resolve){
      ...
      app.use(function (req, res, next) {
         ...
         console.log(build, req.path);

         // get app config
         var appConfPath = server.root + server.appsRoot + build + "/conf.json";
         util.localJson(appConfPath).then(function(appConf){
            var newMount = express();
            newMount.get("/", mount);
            app.use("/" + build, newMount);
         }, function(err){
            console.error(err);
            res.status(500).send("Unknown: " + build);
         });
