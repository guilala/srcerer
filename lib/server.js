...
const enableMountPoints = function(done) {
   const configurationQue = util.all(this);
   ...
   .on("data", (item) => {
      ...
      if(fileName === "app.json"){
         mount(configurationQue, item.path, build, "app");
      }

      else if(fileName === "io.json"){
         mount(configurationQue, item.path, io, "io");
      }

      else if(fileName === "socket.json"){
         mount(configurationQue, item.path, socket, "socket");
      }
   })

   .on("end", ()=> {
      configurationQue.then(done);
   });
};
